...
## Open TCP Ports on Security Group
  resource "aws_security_group_rule" "instances_security_group_allow_ingress_open_tcp_ports" {
    ...
    type              = "ingress"
    from_port         = "${element(var.tcp_ports_to_open_on_instances_security_group, count.index)}"
    to_port           = "${element(var.tcp_ports_to_open_on_instances_security_group, count.index)}"
    protocol          = "tcp"
...
## Open UDP Ports on Security Group
  resource "aws_security_group_rule" "instances_security_group_allow_ingress_open_udp_ports" {
    ...
    type              = "ingress"
    from_port         = "${element(var.udp_ports_to_open_on_instances_security_group, count.index)}"
    to_port           = "${element(var.udp_ports_to_open_on_instances_security_group, count.index)}"
    protocol          = "udp"
    cidr_blocks       = "${var.cidrs_to_open_ports_on_security_groups}"
    security_group_id = "${aws_security_group.instances_security_group.id}"
  }
